...
### Added
- **`MenuTreeBuilder::renderLinks` deep paths** (#61) — two new kernel tests in `EntityHelperMenuTest` covering the two non-trivial branches that the existing flat-menu test didn't reach. `testNestedMenuLinksRecurseIntoBelow` builds a parent + child link, asserts the child is nested under the parent's `below` and that the recursive shape mirrors the top level (so the `renderLinks($item['below'], …)` recursion is exercised). `testFieldFormatterEnrichesMenuItemWithExtras` attaches a custom `field_subtitle` (string) to the `menu_link_content` bundle, creates a link with the value populated, and asserts `getMenu` surfaces the field under `subtitle` (the `field_` prefix stripped) — the same enrichment contract the contrib `menu_item_extras` module relies on. `createMenuLink` helper gained an optional `$parent` argument. Translation-status skip path (`content_translation_status === FALSE`) deferred: requires the heavier `content_translation` module setup; the two branches added here are enough to cross the #61 forecast on their own. Part of v1.5.0 Tier 2 (#55).
||||||| parent of 2eb5cd0 (test(unit): MenuActiveTrailResolver + TaxonomyTreeBuilder helpers (#63))
- **`MenuActiveTrailResolver` + `TaxonomyTreeBuilder` helper coverage** (#63) — three new unit tests. `TaxonomyTreeBuilderTest::testNestedTreeReturnsHierarchicalStructure` exercises the previously-untested `buildTermTree` recursion via `build($vocab, ['nested' => TRUE])` against a 4-level tree (two roots, two children, one grandchild) and pins both branches: nested items carry a `children` array of the same shape and leaves carry an empty `children`. `testNestedTreeReturnsEmptyChildrenForLeaf` locks the `!isset($children_map[$parent_id])` early-return that bounds the recursion. `MenuActiveTrailResolverTest::testSkipsUnroutedBreadcrumbCrumbs` covers the `!$url->isRouted()` branch a custom breadcrumb builder can hit when surfacing external URLs — the resolver skips the crumb without touching the menu link manager and continues to the next deeper crumb. `makeTerm` helper now accepts an optional `$parent` arg so nested-mode fixtures can wire parent/child relationships. Part of v1.5.0 Tier 2 (#55).
